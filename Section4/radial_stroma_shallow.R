...
# Define file paths
data_path <- "D:/Igor/Xenium/Data for clustering algorithm/Z-Filtered Datasets/Alex_20/combined_gene_counts_matrix_normalized_stroma.csv"
...
genes_of_interest <- c("SFRP1","CDCA7L","PDE5A","TGFBI","NDNF","FOXD1","CYP1B1","TRIL")
...
p5 <- ggplot(df_plot, aes(x = continuum, y = r)) +
  geom_polygon(fill = "#1F77B4", color = NA) +  # Solid blue fill, no outline for polygon
...
ggsave("stroma_shallow.tiff", plot = p5, width = 8, height = 8, dpi = 300, device = "tiff",
       compression = "lzw")

message("Nature-ified filled polar plot generated: ", file.path(output_dir, "stroma_shallow.tiff"))