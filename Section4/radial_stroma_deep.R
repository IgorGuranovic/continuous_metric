...
genes_of_interest <- c("GATA6","THBS1","CXCL12","ID2","ID3","TNC")
...
p5 <- ggplot(df_plot, aes(x = continuum, y = r)) +
  geom_polygon(fill = "#FFC0CB", color = NA) +  # Solid blue fill, no outline for polygon
...
ggsave("stroma_deep.tiff", plot = p5, width = 8, height = 8, dpi = 300, device = "tiff",
       compression = "lzw")

message("Nature-ified filled polar plot generated: ", file.path(output_dir, "stroma_deep.tiff"))